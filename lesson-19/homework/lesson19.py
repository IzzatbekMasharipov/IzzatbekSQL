...
with sqlite3.connect("population.db") as conn:
    db=pd.read_sql_query("select * from population",conn)
db   
data=pd.read_excel("population_salary_analysis.xlsx") 
db
def Percentage(salary):
    ...
    elif salary>12000000 and salary<1400000:
        return '$1,200,001 - $1,400,000'
    elif salary>14000000 and salary<1600000:
        return '$1,400,001 - $1,600,000'
    elif salary>16000000 and salary<1800000:
        return '$1,600,001 - $1,800,000'
    elif salary>1800000:
        return '$1,800,001 and over'
    else:
        return 'unknown'
db['salary'].apply(Percentage).value_counts(normalize=True)*100
db['Salary Band']=db['salary'].apply(Percentage)
db

Average_salary=db.groupby('Salary Band')['salary'].mean()
Average_salary


Median_Salary =db.groupby('Salary Band')['salary'].median()
Median_Salary

Number_of_populition=db.groupby('Salary Band')['salary'].count()
Number_of_populition


state_summary=db.groupby(['state','Salary Band'])

state_summary=db.groupby(['state','Salary Band']).agg(salary_count=('salary','count'),salary_mean=('salary','mean'),salary_median=('salary','median'))
state_summary
